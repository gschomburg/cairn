		...
		private void CreateHullMapping()
		{
			...
			foreach (Hull hull in paintingData.hulls)
			{
				if (hullMapping.ContainsKey(hull))
				{
					...
					if (!(isHullOk || isBoxOk || isSphereOk))
					{
			...
			for (int i=orphanedHulls.Count-1; i>=0; i--)
			{
				...
				for (int j=orphanedColliders.Count-1; j>=0; j--)
				{
					...
					bool isMatchingSphere = h.type == HullType.Sphere && c is SphereCollider && Approximately(h.collisionSphere.center, sphereCol.center) && Approximately(h.collisionSphere.radius, sphereCol.radius);
					bool isMatchingMesh = h.type == HullType.ConvexHull && c is MeshCollider && meshCol.sharedMesh == h.collisionMesh;

					if (isMatchingBox || isMatchingSphere || isMatchingMesh)
					{
